...
public interface IPlantEmployeeOperationRepo extends JpaRepository<PlantEmployeeOperation,Long>
{
    ...
    Optional<PlantEmployeeOperation> getEmployeeOperationByEmployeeIdAndDate(@Param("employeeId") String employeeId, @Param("date") LocalDate date);

    @Query("SELECT o FROM PlantEmployeeOperation o WHERE o.employee.employeeId = :employeeId ORDER BY o.operationDate DESC")
    List<PlantEmployeeOperation> getAllOperationsByEmployeeId(@Param("employeeId") String employeeId);
